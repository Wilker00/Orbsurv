...
async function init() {
  ...
  // Check if user has dev access via auth-guard
  // auth-guard.js will handle redirect to login if not authenticated
  try {
    ...
    if (hasAccess) {
      ...
      // auth-guard will redirect to login, but show content as hidden
    ...
    console.warn('Unable to check dev access', error);
...
async function ensureDevAccess({ interactive = false } = {}) {
  ...
  // auth-guard.js will handle this via data-require-dev attribute
  redirectToLogin();
  return false;
}
